...
playMode=$1
configFile="/home/pi/bin/conf"
folder="" #not set yet

read -r configValue<$configFile
...
then
    ...
    then
       ...
       playMode=$configValue

    fi

fi

configValue=$playMode

echo "$playMode" > $configFile
echo "Playmode is: $playMode"
echo "ConfigValue is: $configValue"
...
LOCAL_FILES=/home/pi/Videos/$folder/ # Current folder containing the videos
...
then
        ...
        echo "Getting videos from folder"
	...
	then
		...
		then
	 	...
	 	if [ -f ${VIDS[$PLAYING]} ]; then
			lxterminal -e /usr/bin/omxplayer -r -o hdmi ${VIDS[$PLAYING]} # Play video
		fi
		#echo "Array size= $CURRENT" # error checking code
	else
		echo "Insert USB with videos and restart or add videos to /home/pi/Videos and run ./startvideo.sh"
		exit
	fi
fi
done
